...
def plot_confusion_matrix(cm, classes, normalize=True, title='Confusion matrix', cmap = cmap):
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        plt.text(j, i, format(cm[i, j], fmt),   # for percentage
...
train_path = "/kaggle/input/busi-segmented-mass/BUSI Segmented Mass/Train"
test_path = "/kaggle/input/busi-segmented-mass/BUSI Segmented Mass/Test"
...
total_train_samples = len(glob.glob(train_path+"/Benign/*.png")) + len(glob.glob(train_path+"/Malignant/*.png")) + len(glob.glob(train_path+"/Normal/*.png"))
num_train_samples = np.ceil( total_train_samples * 0.9)
num_val_samples = total_train_samples - num_train_samples
num_test_samples = len(glob.glob(test_path+"/Benign/*.png")) + len(glob.glob(test_path+"/Malignant/*.png")) + len(glob.glob(test_path+"/Normal/*.png"))
...
model_e = models[0]        
model_e.load_weights("/kaggle/input/segmented-mass-classification-weights/efficientnetb7_weights.h5")

model_n = models[1]       
model_n.load_weights("/kaggle/input/segmented-mass-classification-weights/nasnetlarge_weights.h5")

model_r = models[2]       
model_r.load_weights("/kaggle/input/segmented-mass-classification-weights/resnet50v2_weights.h5")
...
stacked_model.load_weights("/kaggle/input/stacked-ensemble-weight/stacked_ensemble_weights.h5")
...
print('auc score: ',sklearn.metrics.roc_auc_score(test_labels_e, predictions, multi_class="ovr", average='macro'))
