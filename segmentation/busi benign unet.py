...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/benign_fold_{fold+1}_unet_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/benign_unet_fold_0"+str(fold+1)+"_weights.h5"
...
model1 = get_unet()
model1.load_weights("/kaggle/input/benign-unet-3-fold-weights/benign_unet_fold_01_weights.h5")
model2 = get_unet()
model2.load_weights("/kaggle/input/benign-unet-3-fold-weights/benign_unet_fold_02_weights.h5")
model3 = get_unet()
model3.load_weights("/kaggle/input/benign-unet-3-fold-weights/benign_unet_fold_03_weights.h5")
...
# imgs_mask_test = ((0.2*img_1) + (0.2*img_2) + (0.6*img_3)) / (0.2 + 0.2 + 0.6)
# imgs_mask_test = (img_1 + img_2 + img_3) / (1 + 1 + 1)
imgs_mask_test = ((0.4*img_1) + (0.2*img_2) + (0.4*img_3)) / (0.2 + 0.4 + 0.4)
...
np.save(os.path.join("/kaggle/working", "benign_imgs_mask_test_unet.npy"), imgs_mask_test)
