...
from tensorflow.keras.layers import Input, concatenate, Conv2D, Add, MaxPooling2D, Activation, Dense, Reshape, GlobalAveragePooling2D, Multiply, Conv2DTranspose, BatchNormalization, Dropout
...
def get_unet():
    ...
    model.compile(optimizer=Adam(), loss=[dice_coef_loss], metrics=[dice_coef, iou_coef, "accuracy"])
    return model
...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/fold_{fold+1}_unet_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/unet_fold_0"+str(fold+1)+"_weights.h5"
...
model1 = get_unet()
model1.load_weights("/kaggle/input/unet-weights-3-fold/unet_fold_01_weights.h5")
model2 = get_unet()
model2.load_weights("/kaggle/input/unet-weights-3-fold/unet_fold_02_weights.h5")
model3 = get_unet()
model3.load_weights("/kaggle/input/unet-weights-3-fold/unet_fold_03_weights.h5")
...
# imgs_mask_test = ((0.2*imgs_mask_test_1) + (0.4*imgs_mask_test_2) + (0.4*imgs_mask_test_3)) / (0.4 + 0.2 + 0.4)
...
for i in range(5):
    ...
    plt.show()
    # fig.savefig(f"/kaggle/working/test_result/{i+1}_benign_unet_test_result.png")
...
gen_imgs_mask[gen_imgs_mask > 0.1] = 1.0
gen_imgs_mask.min()
...
for i in range(5):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/{i+1}_unet_generated_image_result.png")
