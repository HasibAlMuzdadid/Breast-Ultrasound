...
# name = "benign"
# imgs_train, imgs_mask_train = load_train_data("benign")
...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/benign_fold_{fold+1}_cunet_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/benign_cunet_fold_0"+str(fold+1)+"_weights.h5"
...
model1 = get_cunet()
model1.load_weights("/kaggle/input/benign-connected-unet-3-fold-weights/benign connected unet fold weights/benign_cunet_fold_01_weights.h5")
model2 = get_cunet()
model2.load_weights("/kaggle/input/benign-connected-unet-3-fold-weights/benign connected unet fold weights/benign_cunet_fold_02_weights.h5")
model3 = get_cunet()
model3.load_weights("/kaggle/input/benign-connected-unet-3-fold-weights/benign connected unet fold weights/benign_cunet_fold_03_weights.h5")
...
# imgs_mask_test = ((0.9*img_1) + (0.5*img_2) + (0.5*img_3)) / (0.9 + 0.5 + 0.5)


# In[16]:


imgs_mask_test[imgs_mask_test > 0.66] = 1.0
imgs_mask_test[imgs_mask_test <= 0.66] = 0.0
...
for i in range(5):
    ...
    plt.show()
    #fig.savefig(f"/kaggle/working/test_result/{i+1}_benign_cunet_test_result.png")
