...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/fold_{fold+1}_model_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/model_fold_0"+str(fold+1)+"_weights.h5"
...
model1 = get_model()
model1.load_weights("/kaggle/input/model-weights-3-fold/model_fold_01_weights.h5")
model2 = get_model()
model2.load_weights("/kaggle/input/model-weights-3-fold/model_fold_02_weights.h5")
model3 = get_model()
model3.load_weights("/kaggle/input/model-weights-3-fold/model_fold_03_weights.h5")
...
# imgs_mask_test = imgs_mask_test_3
...
imgs_mask_test = ((1.0*imgs_mask_test_1) + (1.0*imgs_mask_test_2) + (0.0*imgs_mask_test_3)) / (1.0 + 1.0 + 0.0)  # image 0.1
...
imgs_mask_test[imgs_mask_test <= 0.1] = 0.0
# imgs_mask_test[imgs_mask_test > 0.4] = 1.0
# imgs_mask_test[imgs_mask_test <= 0.4] = 0.0

imgs_mask_test.min(), imgs_mask_test.max()
...
np.save(os.path.join("/kaggle/working", "imgs_mask_test_model.npy"), imgs_mask_test)
...
for i in range(5):
    ...
    plt.show()
    # fig.savefig(f"/kaggle/working/test_result/{i+1}_model_test_result.png")


# In[31]:


imgs_test[4].shape, imgs_mask_test[4].shape, imgs_id_test[4].shape
