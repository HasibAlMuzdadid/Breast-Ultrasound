...
imgs_train = np.load("/kaggle/input/thammasat-benign/imgs_train_benign.npy")
imgs_mask_train = np.load("/kaggle/input/thammasat-benign/imgs_mask_train_benign.npy")
...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/benign_fold_{fold+1}_model_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/benign_model_fold_0"+str(fold+1)+"_weights.h5"
...
imgs_test = np.load("/kaggle/input/thammasat-benign/imgs_test_benign.npy")
imgs_id_test = np.load("/kaggle/input/thammasat-benign/imgs_id_test_benign.npy")
...
model1 = get_model()
model1.load_weights("/kaggle/input/thammasat-benign-model-3-fold-weights/benign_model_fold_01_weights.h5")
model2 = get_model()
model2.load_weights("/kaggle/input/thammasat-benign-model-3-fold-weights/benign_model_fold_02_weights.h5")
model3 = get_model()
model3.load_weights("/kaggle/input/thammasat-benign-model-3-fold-weights/benign_model_fold_03_weights.h5")
...
imgs_mask_test = ((0.2*img_1) + (0.4*img_2) + (0.4*img_3)) / (0.2 + 0.4 + 0.4)
...
# imgs_mask_test = ((0.5*imgs_mask_test_1) + (0.9*imgs_mask_test_2) + (0.8*imgs_mask_test_3)) / (0.9 + 0.5 + 0.8)
...
np.save(os.path.join("/kaggle/working", "benign_imgs_mask_test_model.npy"), imgs_mask_test)
...
for i in range(20):
    ...
    plt.show()
    #fig.savefig(f"/kaggle/working/test_result/{i+1}_benign_model_test_result.png")
...
data_path = "/kaggle/input/udiat-benign-generated-image/CycleGAN"  
...
pred_dir = "/kaggle/working/udiat_benign_generated_masks"
...
files = glob.glob("/kaggle/input/udiat-benign-generated-image/CycleGAN/*.png") 
len(files)
files[0]
...
sorted_gen_images = sorted(os.listdir("/kaggle/input/udiat-benign-generated-image/CycleGAN/"))
sorted_gen_masks = sorted(os.listdir("/kaggle/working/udiat_benign_generated_masks/"))

# Load the images and masks from the folder
gen_images = [cv2.imread(os.path.join("/kaggle/input/udiat-benign-generated-image/CycleGAN/", img)) for img in sorted_gen_images]
gen_masks = [cv2.imread(os.path.join("/kaggle/working/udiat_benign_generated_masks/", msk)) for msk in sorted_gen_masks]
...
# !zip -r generated_masks.zip /kaggle/working/udiat_benign_generated_masks/
