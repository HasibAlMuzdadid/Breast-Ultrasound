...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/fold_{fold+1}_resunet_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    fname = "/kaggle/working/resunet_fold_0"+str(fold+1)+"_weights.h5"
...
model1 = get_resunet()
model1.load_weights("/kaggle/input/resunet-weights-3-fold-batchnormalization/resunet_fold_01_weights.h5")
model2 = get_resunet()
model2.load_weights("/kaggle/input/resunet-weights-3-fold-batchnormalization/resunet_fold_02_weights.h5")
model3 = get_resunet()
model3.load_weights("/kaggle/input/resunet-weights-3-fold-batchnormalization/resunet_fold_03_weights.h5")
...
# for 0.9, 0.0, 0.1 combination and other combination
# imgs_mask_test[imgs_mask_test > 0.1] = 1.0

# for mask of 0.9, 0.0, 0.1 combination 
# imgs_mask_test[imgs_mask_test < 0.41] = 0.0
# imgs_mask_test[imgs_mask_test >= 0.41] = 1.0

imgs_mask_test[imgs_mask_test < 0.34] = 0.0
imgs_mask_test[imgs_mask_test >= 0.34] = 1.0
imgs_mask_test.min()
...
for i in range(5):
    ...
    plt.show()
    #fig.savefig(f"/kaggle/working/test_result/{i+1}_malignant_resunet_test_result.png")
