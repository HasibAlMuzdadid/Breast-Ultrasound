...
from tensorflow.keras.layers import Input, concatenate, Conv2D, Add, MaxPooling2D, Activation, Dense, Reshape, GlobalAveragePooling2D, Multiply, Conv2DTranspose, BatchNormalization
...
def get_cunet():
    ...
    conv14 = Conv2D(512, (3, 3), activation='relu', padding='same')(pool13)
    ...
    model.compile(optimizer=Adam(1e-4), loss=[dice_coef_loss], metrics=[dice_coef, iou_coef, "accuracy"])
    return model
...
imgs_train = np.load("/kaggle/input/malignant/imgs_train_malignant.npy")
imgs_mask_train = np.load("/kaggle/input/malignant/imgs_mask_train_malignant.npy")
...
def visuals(history):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/fold_{fold+1}_cunet_evaluation.png")
...
for fold, (train_index,val_index) in enumerate(KFold(n_splits = 3, shuffle=False).split(imgs_train,imgs_mask_train)):
    ...
    # fname = "/kaggle/working/cunet_fold_0"+str(fold+1)+"_epoch_{epoch:03d}_acc_{val_accuracy:.4f}_dice_{val_dice_coef:.4f}_iou_{val_iou_coef:.4f}_weights.h5"
    fname = "/kaggle/working/cunet_fold_0"+str(fold+1)+"_weights.h5"

    model_checkpoint = ModelCheckpoint(filepath=fname, monitor="val_loss",mode=min, save_best_only=True,verbose=1)
    # model_reducelr = ReduceLROnPlateau(monitor="val_loss",mode="auto",factor=0.1,patience=15,min_lr=0.00001,verbose=1)
    model_earlystop = EarlyStopping(monitor="val_loss",mode=max,min_delta=0.001,patience=50,verbose=1)
...
imgs_test = np.load("/kaggle/input/malignant/imgs_test_malignant.npy")
imgs_id_test = np.load("/kaggle/input/malignant/imgs_id_test_malignant.npy")
...
model1 = get_cunet()
model1.load_weights("/kaggle/input/connected-unet-weights/cunet_fold_01_weights.h5")
model2 = get_cunet()
model2.load_weights("/kaggle/input/connected-unet-weights/cunet_fold_02_weights.h5")
model3 = get_cunet()
model3.load_weights("/kaggle/input/connected-unet-weights/cunet_fold_03_weights.h5")
model4 = get_cunet()
model4.load_weights("/kaggle/input/connected-unet-weights/cunet_fold_04_weights.h5")
model5 = get_cunet()
model5.load_weights("/kaggle/input/connected-unet-weights/cunet_fold_05_weights.h5")
...
print("Fold 3 done")
imgs_mask_test_4 = model4.predict(imgs_t, verbose=1)
print("Fold 4 done")
imgs_mask_test_5 = model5.predict(imgs_t, verbose=1)
print("Fold 5 done")
...
model1 = get_cunet()
model1.load_weights("/kaggle/input/cunet-weights-3-fold/cunet_fold_01_weights.h5")
model2 = get_cunet()
model2.load_weights("/kaggle/input/cunet-weights-3-fold/cunet_fold_02_weights.h5")
model3 = get_cunet()
model3.load_weights("/kaggle/input/cunet-weights-3-fold/cunet_fold_03_weights.h5")
...
# imgs_mask_test = ((0.0*imgs_mask_test_1) + (0.4*imgs_mask_test_2) + (0.0*imgs_mask_test_3) + (0.2*imgs_mask_test_4) + (0.4*imgs_mask_test_5)) / (0.0 + 0.4 + 0.0 + 0.2 + 0.4)
# imgs_mask_test = ((0.3*imgs_mask_test_1) + (0.3*imgs_mask_test_2) + (0.3*imgs_mask_test_3) + (0.3*imgs_mask_test_4) + (0.3*imgs_mask_test_5)) / (0.3 + 0.3 + 0.3 + 0.3 + 0.3)
# imgs_mask_test = (imgs_mask_test_1 + imgs_mask_test_2 + imgs_mask_test_3+ imgs_mask_test_4 + imgs_mask_test_5) / (1 + 1 + 1 + 1+ 1)
# imgs_mask_test = (imgs_mask_test_1 + imgs_mask_test_2 + (0*imgs_mask_test_3) + (0*imgs_mask_test_4) + (0*imgs_mask_test_5)) / (1 + 1 + 0 + 0+ 0)   # image
imgs_mask_test = ((0.9*imgs_mask_test_1) + (0.5*imgs_mask_test_2) + (0.5*imgs_mask_test_3) + (0.9*imgs_mask_test_4) + (0.5*imgs_mask_test_5)) / (0.9 + 0.5 + 0.5 + 0.9 + 0.5)  # besrt score image
...
# imgs_mask_test = ((0.4*imgs_mask_test_1) + (0.2*imgs_mask_test_2) + (0.4*imgs_mask_test_3)) / (0.4 + 0.2 + 0.4)
# imgs_mask_test = ((0.3*imgs_mask_test_1) + (0.3*imgs_mask_test_2) + (0.3*imgs_mask_test_3)) / (0.3 + 0.3 + 0.3)
# imgs_mask_test = (imgs_mask_test_1 + imgs_mask_test_2 + imgs_mask_test_3) / (1 + 1 + 1)
# imgs_mask_test = ((0.9*imgs_mask_test_1) + (0.5*imgs_mask_test_2) + (0.5*imgs_mask_test_3)) / (0.9 + 0.5 + 0.5)


# In[39]:


# imgs_mask_test[imgs_mask_test > 0.1] = 1.0
...
np.save(os.path.join("/kaggle/working", "imgs_mask_test_cunet.npy"), imgs_mask_test)
...
imgs_mask_test_c = np.load("/kaggle/working/imgs_mask_test_cunet.npy")
imgs_mask_test_c.shape
...
for i in range(5):
    ...
    plt.show()
    #fig.savefig(f"/kaggle/working/test_result/{i+1}_malignant_cunet_test_result.png")
...
data_path = "/kaggle/input/malignant-generated-images"  
...
print("Fold 3 done")
gen_imgs_mask_4 = model4.predict(generated_imgs, verbose=1)
print("Fold 4 done")
gen_imgs_mask_5 = model5.predict(generated_imgs, verbose=1)
print("Fold 5 done")
...
gen_imgs_mask = ((0.9*gen_imgs_mask_1) + (0.5*gen_imgs_mask_2) + (0.5*gen_imgs_mask_3) + (0.9*gen_imgs_mask_4) + (0.5*gen_imgs_mask_5)) / (0.9 + 0.5 + 0.5 + 0.9 + 0.5)
...
pred_dir = "/kaggle/working/malignant_generated_masks"
...
files = glob.glob("/kaggle/input/malignant-generated-images/*.png") 
len(files)
# files[0]
...
sorted_gen_images = sorted(os.listdir("/kaggle/input/malignant-generated-images/"))
sorted_gen_masks = sorted(os.listdir("/kaggle/working/malignant_generated_masks/"))

# Load the images and masks from the folder
gen_images = [cv2.imread(os.path.join("/kaggle/input/malignant-generated-images/", img)) for img in sorted_gen_images]
gen_masks = [cv2.imread(os.path.join("/kaggle/working/malignant_generated_masks/", msk)) for msk in sorted_gen_masks]
...
for i in range(5):
    ...
    plt.show()
    fig.savefig(f"/kaggle/working/{i+1}_cunet_result.png")


# In[ ]:


# !zip -r generated_masks.zip /kaggle/working/malignant_generated_masks/
